...
module.exports = function readTypeScriptModules(
  ...
  function convertHeritageClauseToSymbols(heritage, typeChecker) {
    var heritages = heritage.types.map(expression => {
      if (!typeChecker.getSymbolAtLocation(expression)) {
        console.log(util.inspect(expression, { colors: true, depth: 2}));
      }
      return typeChecker.getSymbolAtLocation(expression).getSymbol()
    });
